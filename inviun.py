...
# Constants
MAX_FILE_SIZE = 5 * 1024 * 1024  # 5MB limit for Anthropic API
...
async def generate_captions(content: list, platform: str):
    ...
    This implementation provides a simple local fallback so the bot remains
    functional even without an external API key. If you want to use a remote
    LLM (Anthropic/OpenAI), replace or extend this function accordingly.
    """
    ...
    # Local simple generator: create 3 variations using templates
    captions = []
    hooks = [
